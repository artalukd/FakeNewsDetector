...
def get_contentListShow(url):
	...
	article = soup.find("div",class_= "mh-wrapper clearfix")
	...
	while(index!=-1 and len(arrbit) < 1000):
		...
		zarticle = zsoup.find("div",class_="mh-main clearfix")
		t = ""
		p = ""
		try:
		...
		if(c!='none' and z not in arrlink and z.find("http")!=-1 and z.find("http://indianexpress.com/section/india/politics/page/")==-1 and z.find("http://teekhimirchi.in/")!=-1 and z.find(">")==-1 and z.find("\"")==-1):		
			arrtitle.append(t)
			arrcontent.append(c)
			arrlink.append(z)
			print z
			arrbit.append(1)
			print len(arrbit)
			filobj = open("teekhimirchi.txt",'wb')
...
def main():
	url = "http://teekhimirchi.in/politics-en/page/"
	num = 1
