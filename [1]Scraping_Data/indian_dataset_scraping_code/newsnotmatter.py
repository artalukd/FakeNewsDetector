...
def get_contentListShow(url):
	...
	article = soup.find("div",class_= "vce-loop-wrap")
	contentnew = str(article)
	index = contentnew.find("href=")
	while(index!=-1 and len(arrbit) < 1000):
		...
		zarticle = zsoup.find("div",class_="vce-main-content")
		t = ""
		p = ""
		try:
		...
		if(c!='none' and c!="" and z not in arrlink and z.find("http")!=-1 and z.find("http://newsthatmattersnot.com/category/the-political-drama/page/")==-1 and z.find("http://newsthatmattersnot.com/")!=-1 and z.find(">")==-1 and z.find("\"")==-1):		
			arrtitle.append(t)
			print t
			arrcontent.append(c)
			print c
			...
			arrbit.append(1)
			print len(arrbit)
			filobj = open("newsnotmatter.txt",'wb')
...
def main():
	url = "http://newsthatmattersnot.com/category/the-political-drama/page/"
	num = 1
